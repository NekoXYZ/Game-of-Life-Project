...
public class version0_4_4
{
    ...
    void load(){
        ...
        String valuesToLoad[][]=new String[WIDTH][HEIGHT];//keep all lines that are read in
        ...
        try {
            ...
            for (int x=0; x<lineCount; x++){ 
                String values[] = SavedLines[x].split(","); //break up each line from the scanner at each ","
                for(int y=0; y<values.length;y++){
                    valuesToLoad[x][y]=values[y]; //sets the value of the loading array at [x(the line we are on)][y(the position in that line)] to be equal to that saved in the csv file
                }
            }

        } catch (IOException e) {System.out.println(e);}

        for (int x=0; x<WIDTH;x++){  //set the current board to be equal to the loading array
            for (int y=0; y<HEIGHT; y++){
                currentBoard[x][y] = valuesToLoad[x][y];
            }
        }
    }
